...
public class EnemyMeleeController : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        targetDistance = target.position - this.transform.position;

        if (!isDead)
        {
            //if (detection != null)
            //{
            //    if (detection.playerInArea == true)
            //    {
                        ...
                        Vector2 movement = targetDistance * currentSpeed * Time.fixedDeltaTime;
                        rb.linearVelocity = movement + rb.linearVelocity;

                        // ATAQUE
                        // Se estiver perto do Player e o timer do jogo for maior que o valor de nextAttack
                        if (Mathf.Abs(targetDistance.x) < 1.5f && Mathf.Abs(targetDistance.y) < 0.05f && Time.time > nextAttack)
                        {
